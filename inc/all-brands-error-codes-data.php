...
function ar_error_codes_viking_wall_oven(): array {
    ...
    return [
        ...
        $base + ['title' => 'Viking Wall Oven F1 Fault Code', 'slug' => 'viking-wall-oven-f1-fault-code',
            ...
            'meta_desc'  => 'The Viking wall oven F1 fault code indicates a main control board (ERC) failure. Learn causes, reset steps, and when professional repair is required.',
            'meta_fields' => ['_ar_brand' => 'Viking', '_ar_appliance_type' => 'Wall Oven', '_ar_error_code' => 'F1',
                '_ar_code_meaning'   => "F1 on a Viking wall oven indicates the main Electronic Range Control (ERC) board has detected an internal fault and shut down oven operation. Viking wall ovens use the same ERC architecture as Viking ranges — F-codes carry identical meaning across both product types. F1 is typically caused by a power surge, age-related component failure, or heat cycling degrading internal relay or capacitor components.\n\nAttempt a reset by switching off the dedicated circuit breaker for 60 seconds. If F1 returns after reset, the ERC board requires professional replacement with a genuine Viking OEM part.",
                '_ar_causes'         => [['title' => 'Control Board Internal Failure', 'description' => 'Failed relay, capacitor, or microprocessor on the ERC caused by age, heat cycling, or voltage irregularity from the power supply circuit.'], ['title' => 'Power Surge or Voltage Spike', 'description' => 'A surge during a thunderstorm or utility grid event can damage sensitive ERC components, causing F1 to appear immediately after power is restored to the wall oven.'], ['title' => 'Overheating from Restricted Installation Ventilation', 'description' => 'Insufficient clearance around the wall oven cabinet or blocked ventilation paths can cause ERC operating temperatures to exceed design limits, leading to premature board component failure.'], ['title' => 'Loose or Corroded Wiring Harness Connector', 'description' => 'A corroded or unseated ERC wiring harness connector produces an intermittent F1 that temporarily clears after a power reset before returning on the next oven use.']],
                '_ar_diy_steps'      => [['title' => 'Circuit Breaker Reset', 'description' => 'Turn off the dedicated wall oven circuit breaker for 60 seconds, then restore power. If F1 does not return immediately, monitor the oven over the next several cooking cycles before concluding the fault was transient.'], ['title' => 'Verify Circuit Breaker Condition', 'description' => 'Confirm the wall oven circuit breaker is at the correct amperage rating and is not in a partially tripped state. A loose or undersized circuit connection can deliver unstable voltage that gradually damages the ERC over time.'], ['title' => 'Record the Fault Context', 'description' => 'Note whether F1 appeared during a cooking cycle, on cold power-up, or immediately after a power outage or restoration event. This diagnostic context helps your technician distinguish surge damage from age-related failure.']],
                '_ar_when_to_call'   => 'F1 codes that return after circuit breaker reset require professional ERC diagnosis. Our technician checks wiring harness integrity and supply voltage stability before recommending board replacement, completing the repair in a single visit with a Viking OEM ERC.',
                '_ar_cost_range'     => '$250 – $480',
                '_ar_severity'       => 'High',
                '_ar_faqs'           => [['question' => 'Are Viking wall oven and range F-codes the same?', 'answer' => 'Yes. Viking wall ovens and ranges use identical ERC systems. F-codes have the same meaning across both product types and are repaired identically — the specific board part number differs by model series only.'], ['question' => 'Can a power surge cause F1 on a Viking wall oven?', 'answer' => 'Yes. Power surges are a frequent cause of ERC failure. If F1 appeared immediately after a thunderstorm, utility outage, or power restoration event, a voltage surge likely damaged sensitive ERC components. Installing a whole-home surge protector helps prevent recurrence.'], ['question' => 'How long does Viking wall oven control board replacement take?', 'answer' => 'ERC replacement is typically completed on a single service visit in 1–2 hours. Our technician sources the correct OEM ERC for your Viking wall oven model series, performs the replacement, and confirms all oven functions before completing the call.']]]],
        ...
        $base + ['title' => 'Viking Wall Oven F2 Fault Code', 'slug' => 'viking-wall-oven-f2-fault-code',
            ...
            'meta_desc'  => 'The Viking wall oven F2 fault code means the oven RTD temperature sensor is reading a short circuit. Learn causes, DIY reset steps, and repair.',
            'meta_fields' => ['_ar_brand' => 'Viking', '_ar_appliance_type' => 'Wall Oven', '_ar_error_code' => 'F2',
                '_ar_code_meaning'   => "F2 on a Viking wall oven indicates the oven temperature sensor (RTD probe) is reading a short circuit — sensor resistance is below the acceptable minimum for the current oven temperature. The ERC interprets this as a dangerously high temperature reading and shuts down the oven as a safety measure. Do not use the wall oven with F2 active — the oven cannot accurately regulate heat.\n\nF2 is most commonly caused by a failed RTD probe, which is a straightforward OEM replacement. Replacement with a genuine Viking OEM sensor resolves F2 in the majority of cases.",
                '_ar_causes'         => [['title' => 'Failed RTD Sensor Probe (Internal Short)', 'description' => 'The resistance temperature detector probe has failed internally with an electrical short, producing a near-zero resistance reading that the ERC interprets as an impossibly high temperature.'], ['title' => 'Sensor Wiring Short Circuit', 'description' => 'The wiring between the RTD probe and the ERC has developed a short — typically from the harness chafing against the oven cavity wall or rear panel during a service event.'], ['title' => 'Moisture Contamination of Sensor Connector', 'description' => 'Steam or condensate inside the sensor connector has created a conductive bridge between the signal terminals, simulating a shorted probe element.'], ['title' => 'Heat Damage to Probe Wiring Insulation', 'description' => 'Extended high-temperature oven use has degraded the sensor wiring insulation, causing the two signal conductors to contact each other and create a short-circuit condition.']],
                '_ar_diy_steps'      => [['title' => 'Reset the Wall Oven', 'description' => 'Switch off the wall oven circuit breaker for 60 seconds, then restore power. If F2 does not immediately return, monitor the oven through one complete bake cycle to confirm the fault has cleared.'], ['title' => 'Do Not Use the Oven Until Repaired', 'description' => 'F2 means the oven cannot accurately read or regulate its own temperature. Operating the wall oven with an unresolved F2 fault risks overheating the oven cavity and damaging food and internal components.'], ['title' => 'Locate and Inspect the RTD Probe', 'description' => 'With power off and the oven cooled, inspect the RTD probe at the back of the oven cavity for visible damage or corrosion. Check the probe connector for moisture or contamination. Report your observations to your technician to assist in diagnosis.']],
                '_ar_when_to_call'   => 'F2 codes that return after a breaker reset require professional RTD probe diagnosis. Our technician tests sensor resistance with a calibrated multimeter and replaces the probe with a genuine Viking OEM part matched to your wall oven model series.',
                '_ar_cost_range'     => '$130 – $230',
                '_ar_severity'       => 'Medium',
                '_ar_faqs'           => [['question' => 'Can I use my Viking wall oven with the F2 error code active?', 'answer' => 'No. F2 means the temperature sensor is shorted and the oven cannot regulate heat accurately. Using the oven with an active F2 fault risks overheating the cavity. Do not use the wall oven until the RTD probe has been replaced by a certified technician.'], ['question' => 'What is the difference between F2 and F3 on a Viking wall oven?', 'answer' => 'F2 indicates the RTD sensor is reading a short circuit — resistance too low, indicating an impossibly high temperature. F3 indicates an open circuit — resistance too high or infinite, indicating the sensor is broken or disconnected. Both prevent oven operation as safety measures, and both are resolved by replacing the RTD probe or its wiring.'], ['question' => 'How do I know if my Viking wall oven RTD probe needs replacement?', 'answer' => 'A technician will test the probe resistance with a multimeter. A functional Viking RTD probe reads approximately 1,090–1,100 ohms at 70°F room temperature. A reading near zero ohms confirms the probe is shorted (F2); a reading of infinite resistance confirms the probe is open (F3). Both conditions require OEM probe replacement.']]]],

        $base + ['title' => 'Viking Wall Oven F3 Fault Code', 'slug' => 'viking-wall-oven-f3-fault-code',
            'meta_title' => 'Viking Wall Oven F3 Fault Code — Oven Temperature Sensor Open Circuit',
            'meta_desc'  => 'The Viking wall oven F3 fault code indicates the oven temperature sensor has an open circuit. Learn causes, DIY reset steps, and repair.',
            'meta_fields' => ['_ar_brand' => 'Viking', '_ar_appliance_type' => 'Wall Oven', '_ar_error_code' => 'F3',
                '_ar_code_meaning'   => "F3 on a Viking wall oven indicates the oven temperature sensor (RTD probe) circuit is open — either the probe itself has fractured internally or its wiring connector has come loose. The ERC reads an infinite resistance value from the sensor circuit and shuts down the oven as a safety measure. F3 and F2 are diagnosed with the same process: the technician tests RTD probe resistance to identify whether the probe, connector, or wiring is the cause.\n\nDo not operate the wall oven while F3 is active — the oven cannot measure or regulate its own temperature without a functioning RTD probe.",
                '_ar_causes'         => [['title' => 'Failed RTD Sensor Probe (Internal Open Circuit)', 'description' => 'The resistance element inside the RTD probe has fractured, reading infinite resistance at the ERC. The probe must be replaced with a genuine Viking OEM part matched to the wall oven model series.'], ['title' => 'Disconnected Sensor Wiring Connector', 'description' => 'The connector joining the RTD probe wiring to the oven harness has vibrated loose or corroded, opening the sensor circuit without the probe itself failing.'], ['title' => 'Damaged Sensor Wiring Harness', 'description' => 'Heat exposure or physical damage during a prior service event has broken a conductor in the sensor wiring harness, opening the circuit between the probe and the ERC.'], ['title' => 'Failed ERC Sensor Input Circuit', 'description' => 'In rare cases, the ERC sensor input circuit has failed internally, causing the board to read an open circuit even with a functional, properly connected probe installed.']],
                '_ar_diy_steps'      => [['title' => 'Reset the Wall Oven', 'description' => 'Switch off the wall oven circuit breaker for 60 seconds, then restore power. If F3 does not immediately return, monitor the oven through one complete bake cycle to confirm the fault has cleared.'], ['title' => 'Inspect the RTD Probe and Connector', 'description' => 'With power off and the oven cooled, inspect the RTD probe inside the oven cavity for cracking, corrosion, or physical damage. Trace the probe wire to its connector and confirm it is fully seated with no visible corrosion on the terminals.'], ['title' => 'Do Not Use the Oven Until Repaired', 'description' => 'The wall oven cannot regulate temperature with an open sensor circuit. Do not attempt to cook until a technician has tested probe resistance and replaced the probe or harness as needed.']],
                '_ar_when_to_call'   => 'F3 codes that return after a breaker reset require professional probe resistance testing. Our technician measures RTD ohm values, inspects the connector, and replaces the probe with a Viking OEM part in a single service visit.',
                '_ar_cost_range'     => '$130 – $230',
                '_ar_severity'       => 'Medium',
                '_ar_faqs'           => [['question' => 'Is F3 on a Viking wall oven the same as F3 on a Viking range?', 'answer' => 'Yes. Viking wall ovens use the same ERC system as Viking ranges. F3 means an open RTD sensor circuit in both products and is repaired identically — replacing the probe or reseating its connector.'], ['question' => 'Can I test the RTD probe on my Viking wall oven myself?', 'answer' => 'Yes, with a multimeter. A functional RTD probe reads approximately 1,090–1,100 ohms at room temperature (70°F). A reading of infinite resistance (open circuit) confirms the probe has failed. A reading near zero ohms indicates a short (F2 condition). Share your reading with your technician for faster diagnosis.'], ['question' => 'Is F3 on a Viking wall oven dangerous?', 'answer' => 'F3 itself is a protective fault — the ERC shuts the oven down specifically to prevent it from operating without temperature feedback. The oven is not in an unsafe state, but it cannot function until the probe is replaced. Do not attempt to bypass the fault protection.']]]],
        ...
        $base + ['title' => 'Viking Wall Oven F4 Fault Code', 'slug' => 'viking-wall-oven-f4-fault-code',
            ...
            'meta_desc'  => 'The Viking wall oven F4 fault code triggers when the oven exceeds the set temperature by more than 50°F. Learn the causes, safety steps, and repair.',
            'meta_fields' => ['_ar_brand' => 'Viking', '_ar_appliance_type' => 'Wall Oven', '_ar_error_code' => 'F4',
                '_ar_code_meaning'   => "F4 on a Viking wall oven triggers when the oven cavity temperature exceeds the programmed set point by more than 50°F. The ERC detects the runaway heating condition and shuts down the oven as a critical safety measure. F4 is most commonly caused by a relay on the ERC that has welded its contacts in the closed position, causing the bake or broil element to remain continuously energized past the set point.\n\nF4 is a critical safety fault. Do not use the wall oven until a certified technician has diagnosed and resolved the root cause. A stuck relay creates an overtemperature hazard that can damage the oven interior and surrounding cabinetry.",
                '_ar_causes'         => [['title' => 'Stuck Closed Relay on ERC', 'description' => 'The relay controlling the bake or broil element has welded its contacts in the closed (energized) position, keeping the element on continuously. The oven overheats until the ERC detects the runaway condition and triggers F4.'], ['title' => 'RTD Sensor Reporting False Low Temperature', 'description' => 'A degraded RTD probe is reporting a lower temperature than actual, causing the ERC to continue commanding heat even as the true cavity temperature rises well above the set point.'], ['title' => 'Relay Board Control Circuit Fault', 'description' => 'On models with a separate relay board, a fault in the relay driver circuit can cause a relay to assert continuously without receiving a command from the ERC.'], ['title' => 'Wiring Fault Bypassing Relay Control', 'description' => 'A wiring short between the element power circuit and the live supply, bypassing the relay entirely, allows continuous current to flow to the heating element regardless of relay position.']],
                '_ar_diy_steps'      => [['title' => 'Turn Off the Wall Oven at the Circuit Breaker Immediately', 'description' => 'Switch off the wall oven circuit breaker. F4 indicates the oven has already exceeded a safe temperature threshold. Keep the breaker off until a professional diagnosis is complete.'], ['title' => 'Do Not Reset and Retry', 'description' => 'Unlike transient faults, F4 should not be cleared by a power reset and reattempted. If a relay is welded closed, the oven will overheat again immediately on next use. A power reset does not fix a failed relay.'], ['title' => 'Document Fault Conditions', 'description' => 'Record the oven set temperature, the mode (Bake, Broil, or Convection), and how long the oven had been running before F4 appeared. This diagnostic information helps your technician quickly identify whether the ERC relay or temperature sensor is the primary fault.']],
                '_ar_when_to_call'   => 'F4 requires immediate professional service. Do not operate the wall oven with a recurring F4 fault. Our technician determines whether the ERC relay, relay board, or temperature sensor is the root cause before replacing any component.',
                '_ar_cost_range'     => '$180 – $480',
                '_ar_severity'       => 'Critical',
                '_ar_faqs'           => [['question' => 'Should I be concerned about F4 on my Viking wall oven?', 'answer' => 'Yes — F4 is a critical safety fault. It indicates the oven overheated past a safe threshold. A stuck relay can cause the oven interior, food, and surrounding cabinetry to reach dangerous temperatures. Do not use the wall oven until it has been professionally diagnosed and repaired.'], ['question' => 'Can F4 damage my Viking wall oven or cabinets?', 'answer' => 'Yes. A runaway oven temperature event can discolor or warp the oven cavity interior, damage door gaskets, and in extreme cases, affect the surrounding cabinetry. This is why the ERC shuts down the oven when F4 is detected rather than allowing it to continue heating.'], ['question' => 'Is F4 the same on a Viking wall oven and Viking range?', 'answer' => 'Yes. Viking wall ovens and ranges use identical ERC systems. F4 has the same meaning — oven temperature runaway — in both product types. The diagnosis and repair procedure are also identical.']]]],
        ...
        $base + ['title' => 'Viking Wall Oven F7 Fault Code', 'slug' => 'viking-wall-oven-f7-fault-code',
            ...
            'meta_desc'  => 'The Viking wall oven F7 fault code means a keypad button is registering as permanently pressed. Learn causes, DIY drying steps, and when to call for repair.',
            'meta_fields' => ['_ar_brand' => 'Viking', '_ar_appliance_type' => 'Wall Oven', '_ar_error_code' => 'F7',
                '_ar_code_meaning'   => "F7 on a Viking wall oven means the ERC has detected a keypad button registering as continuously pressed — a shorted or stuck key condition. The wall oven cannot accept valid user commands while F7 is active.\n\nF7 is most commonly caused by moisture from steam or a spill reaching the membrane keypad and shorting a key contact. If F7 appeared after cleaning, steam exposure, or a nearby boiling pot, allow the control panel to dry completely for 24–48 hours before resetting. If F7 appeared without any moisture event and returns after reset, the membrane switch assembly has failed and requires professional replacement.",
                '_ar_causes'         => [['title' => 'Moisture Infiltration into Membrane Keypad', 'description' => 'Steam from cooking — including from a nearby rangetop burner or the wall oven itself — or liquid from cleaning has entered the membrane keypad and is shorting a key contact to register as continuously pressed.'], ['title' => 'Failed Membrane Switch Layer', 'description' => 'The conductive membrane layer beneath the keypad surface has deteriorated, cracked, or been punctured, creating a permanent short on one or more control inputs without any moisture present.'], ['title' => 'Grease Accumulation Under Keypad Edge', 'description' => 'Grease that has worked under the edge of the keypad membrane has built up enough to depress or bridge a contact area, registering as a stuck key.'], ['title' => 'Ribbon Cable Fault', 'description' => 'The ribbon cable connecting the keypad membrane to the control board has developed a pinched or shorted trace, causing the ERC to receive a continuous input signal mimicking a stuck key.']],
                '_ar_diy_steps'      => [['title' => 'Turn Off the Wall Oven and Allow Panel to Dry', 'description' => 'Switch off the wall oven and turn off the circuit breaker. If F7 appeared after a spill or steam exposure, allow the control panel area to dry completely at room temperature for at least 24–48 hours before attempting a reset.'], ['title' => 'Circuit Breaker Reset After Drying', 'description' => 'After the panel has dried, restore circuit breaker power. If F7 clears and does not immediately return, the issue was moisture-related. Monitor the panel over the next several uses to confirm the fault has resolved.'], ['title' => 'Clean the Control Panel Surface', 'description' => 'With the oven off, use a slightly damp cloth to clean around all keypad button edges and seams where grease and residue accumulate. Avoid excess moisture. Allow to dry fully before testing.']],
                '_ar_when_to_call'   => 'If F7 returns after the control panel has been fully dried and the breaker reset, the keypad membrane switch or control panel assembly has failed and requires professional replacement with a Viking OEM part.',
                '_ar_cost_range'     => '$160 – $320',
                '_ar_severity'       => 'Medium',
                '_ar_faqs'           => [['question' => 'Can cooking steam cause F7 on a Viking wall oven?', 'answer' => 'Yes. Steam from a nearby rangetop, the wall oven itself during high-moisture cooking, or aggressive steam cleaning near the control panel can reach the membrane keypad and cause F7. Allow the panel to dry completely for 24–48 hours and reset the breaker.'], ['question' => 'My Viking wall oven shows F7 without any spill — what does that mean?', 'answer' => 'F7 without a moisture event indicates the membrane switch layer has failed due to age, wear, or a ribbon cable fault — not moisture. In this case, drying will not resolve the issue and professional keypad membrane replacement is required.'], ['question' => 'Can I still use my Viking wall oven with an F7 fault code?', 'answer' => 'F7 prevents the ERC from accepting valid user commands since it interprets the stuck key as continuous input. The oven will not function normally until F7 is cleared. Allow the panel to dry and reset, and if F7 persists, schedule professional service.']]]],
        ...
        $base + ['title' => 'Viking Wall Oven F9 Fault Code', 'slug' => 'viking-wall-oven-f9-fault-code',
            ...
            'meta_desc'  => 'The Viking wall oven F9 fault code means the self-clean door lock failed to engage or disengage. Learn causes, safe unlock steps, and repair.',
            'meta_fields' => ['_ar_brand' => 'Viking', '_ar_appliance_type' => 'Wall Oven', '_ar_error_code' => 'F9',
                '_ar_code_meaning'   => "F9 on a Viking wall oven indicates the self-clean door lock mechanism failed to complete its latch cycle — either the lock did not engage before the self-clean cycle could start, or it failed to disengage after the cycle ended. Viking wall ovens require confirmed door lock engagement before the self-clean cycle will run at pyrolytic temperatures.\n\nIf F9 appears with the door locked, switch off the circuit breaker for 5 full minutes to attempt a lock motor reset. Never force a locked wall oven door — doing so can deform the latch rod and create a more costly repair. If the door remains locked after a reset, professional service is required.",
                '_ar_causes'         => [['title' => 'Failed Door Lock Motor Assembly', 'description' => 'The motor that drives the lock bolt has failed mechanically or electrically and cannot complete its travel to the locked or unlocked position within the ERC-allowed time.'], ['title' => 'Door Lock Position Switch Failure', 'description' => 'The microswitch that reports lock position to the ERC has failed, so the board never receives confirmation that the latch completed its cycle — even if the motor traveled correctly.'], ['title' => 'Food Debris Obstructing Lock Mechanism', 'description' => 'Accumulated food residue in the door latch track or strike plate area is physically preventing the lock bolt from fully engaging or retracting. Cleaning the latch area may resolve this cause.'], ['title' => 'Wiring Fault to Lock Motor Assembly', 'description' => 'An open or intermittent wiring connection to the door lock motor assembly prevents the motor from receiving the voltage needed to complete its travel.']],
                '_ar_diy_steps'      => [['title' => 'Circuit Breaker Reset (if door is locked)', 'description' => 'Switch off the wall oven circuit breaker for 5 full minutes, then restore power. The lock motor should attempt to return to the unlocked home position on power-up. Test the door handle gently after the reset completes.'], ['title' => 'Clean the Door Latch Area', 'description' => 'If the door is accessible (unlocked state), inspect and clean the door latch strike plate and lock mechanism track for accumulated food residue. Grease and food buildup can mechanically obstruct the lock bolt travel. Retry the self-clean cycle after cleaning.'], ['title' => 'Do Not Force the Door', 'description' => 'Never attempt to pull, pry, or force the wall oven door if it is in the locked position. Forcing the mechanism can bend the lock rod and substantially increase the cost of repair. Call for professional service if the breaker reset does not release the door.']],
                '_ar_when_to_call'   => 'F9 that returns consistently after a breaker reset, or any case where the door remains locked after a 5-minute reset, requires professional door lock motor or switch replacement with a genuine Viking OEM assembly.',
                '_ar_cost_range'     => '$150 – $270',
                '_ar_severity'       => 'High',
                '_ar_faqs'           => [['question' => 'How do I open my locked Viking wall oven with an F9 error?', 'answer' => 'Switch off the wall oven circuit breaker for 5 full minutes, then restore power. The lock motor will attempt to reset to the unlocked home position on power-up. If the door remains locked after the reset, do not force it — contact a Viking-authorized technician immediately.'], ['question' => 'Can I use my Viking wall oven normally while the F9 fault is active?', 'answer' => 'F9 blocks self-clean mode specifically, since that cycle requires a confirmed locked door. Normal baking and broiling may remain available if the door is in the unlocked state when F9 appears. However, the door lock mechanism should be repaired before the next self-clean attempt.'], ['question' => 'Is the F9 fault the same on a Viking wall oven and Viking range?', 'answer' => 'Yes. Viking wall ovens and ranges share the same ERC and door lock architecture. F9 has the same meaning in both products — the self-clean door lock failed to complete its cycle — and is repaired identically.']]]],
